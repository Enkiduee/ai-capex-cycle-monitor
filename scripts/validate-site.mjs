...
assert(app.includes('function renderMarketTurnover(data)'), '前端必须渲染中美市场每日成交额');
assert(html.includes('id="market-turnover-chart"') && html.includes('id="market-turnover-tech-chart"') && html.includes('id="turnover-history-body"'), '每日成交额视图必须包含全市场、科技趋势图与历史明细表');
assert(charts.includes('initTurnoverChart') && charts.includes('initTurnoverTechChart') && charts.includes("commonValueAxis('亿美元')") && charts.includes('rollingYearStartDate'), '图表模块必须以每日汇率和亿美元绘制最近一年全市场与科技成交额');
...
assert(html.includes('id="stock-watchlist-body"'), '供应链视图必须包含分类股票观察池');
assert(html.includes('data-stock-market="cn"') && html.includes('data-stock-market="hk"') && html.includes('data-stock-market="us"'), '股票观察池必须提供沪深、港股和美股分类');
assert(html.includes('id="stock-watchlist-search"'), '股票观察池必须支持按名称或代码搜索');
...
assert(app.includes('renderStockWatchlistTable'), '前端必须渲染分类股票观察池');
assert(styles.includes('.market-tabs button[aria-selected="true"]'), 'CSS 必须显示当前股票市场分类');
assert(styles.includes('.stock-watchlist-table'), 'CSS 缺少股票观察池表格样式');
assert(styles.includes('.stock-directory-value') && styles.includes('.stock-directory-link'), 'CSS 缺少股票资料目录样式');
assert(html.includes('股票资料目录') && html.includes('交易币种'), '股票观察池必须按基础资料目录展示');
assert(!html.includes('stock-watchlist-notice') && !app.includes('截图盘前'), '股票资料目录不得展示截图行情口径');
