...
assert(app.includes("stockWatchlist: { path: './data/stock-watchlist.json'"), '前端必须加载独立的股票观察池快照');
assert(app.includes("hkWatchlist: { path: './data/hk-watchlist.json'"), '前端必须加载港股截图观察池');
assert(app.includes("usWatchlist: { path: './data/us-watchlist.json'"), '前端必须加载美股截图观察池');
assert(app.includes('stockWatchlistUniverse'), '前端必须把截图标的与重点研究标的合并为分类观察池');
assert(app.includes('state.data.hkWatchlist'), '分类观察池必须合并港股截图数据');
assert(app.includes('state.data.usWatchlist'), '分类观察池必须合并美股截图数据');
...
assert(styles.includes('.stock-premarket-price') && styles.includes('.stock-premarket-move'), 'CSS 缺少美股盘前行情样式');
